...
public class MenuManager : MonoBehaviour
{
    ...
    {
        pane1.SetActive(true);                                                                                      //啟動設定(布林值) - true 顯示     false 隱藏
        ...
        //while (ao.isDone == true)  簡寫 ao.isDone
        //while (ao.isDone == false)  簡寫 !ao.isDone
        //當 載入資訊.完成 為 false - 尚未載入完成時執行迴圈
        while (!ao.isDone )
        {
            //progress 載入場景的進度值為 0 - 1，如果設定allow 為false 會卡在 0.9
            ...
            if (ao.progress == 0.9f)                                                                                                             //如果 ao.進度 等於0.9
            {
                textTip.enabled = true;                                                                                                          //題是文字.啟動 = 是 - 顯示提是文字
